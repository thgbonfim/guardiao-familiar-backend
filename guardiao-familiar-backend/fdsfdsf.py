...
async def vigia_de_remedios():
    while True:
        ...
        try:
            ...
            # 1. Busca todos os remédios do banco
            response = supabase.table("remedios").select("*").execute()
            if response.error:
                raise Exception(f"Erro ao buscar remédios: {response.error}")

            for remedio in response.data:
                ...
                if dia_da_semana_hoje in remedio["dias_da_semana"]:
                    ...
                    if agora > (horario_agendado_hoje + timedelta(minutes=15)):
                        ...
                        confirmacoes = supabase.table("confirmacoes") \
                            .select("id") \
                            .eq("id_do_remedio", remedio['id']) \
                            .gte("data_confirmacao", data_hoje_str) \
                            .execute()

                        if len(confirmacoes.data) == 0:
                            # 5. Buscar nome do parente e família
                            parente_info = supabase.table("parentes").select("nome, id_da_familia") \
                                .eq("id", remedio["id_do_parente"]).single().execute()

                            familia_info = None
                            if parente_info.data:
                                familia_info = supabase.table("familias").select("nome_da_familia") \
                                    .eq("id", parente_info.data["id_da_familia"]).single().execute()

                            nome_parente = parente_info.data["nome"] if parente_info and parente_info.data else "N/A"
                            nome_familia = familia_info.data["nome_da_familia"] if familia_info and familia_info.data else "N/A"

                            # 6. Dispara o Alerta
                            print("\n" + "!"*60)
                            print(f"### ALERTA (Família: {nome_familia}) ###")
                            print(f"### O parente '{nome_parente}' PODE TER ESQUECIDO o remédio '{remedio['nome_do_remedio']}' das {remedio['horario']}! ###")
                            print("!"*60 + "\n")

                            # 7. Marca no banco que o alerta foi enviado
                            supabase.table("confirmacoes").insert({
                                "id_do_remedio": remedio['id'],
